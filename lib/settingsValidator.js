...
function validateSettings(settings) {
  ...
  if (settings.customInstructions !== undefined) {
    ...
    if (settings.customInstructions.length > 5000) {
      throw new Error('customInstructions must be 5000 characters or less');
    }
  }

  // Validate pageRange
  if (settings.pageRange !== undefined) {
    if (typeof settings.pageRange !== 'string') {
      throw new Error('pageRange must be a string');
    }
    if (settings.pageRange.length > 200) {
      throw new Error('pageRange must be 200 characters or less');
    }
    // Validate format if not empty
    if (settings.pageRange.trim() !== '') {
      const pageRangePattern = /^(\d+(-\d+)?)(,\s*\d+(-\d+)?)*$/;
      if (!pageRangePattern.test(settings.pageRange.trim())) {
        throw new Error('pageRange must be in format like "1-5, 7, 9-12" or empty');
      }
    }
  }
