...
function buildConversionPrompt(settings, workDir) {
  ...
  let prompt = `Convert this PDF to DOCX format using JavaScript and the docx library.
...
4. Generate COMPLETE executable JavaScript code that:
   - Starts with: const { Document, Paragraph, Packer, ... } = require('docx');
   - Includes: const fs = require('fs');
...
function validateGeneratedCode(code) {
  ...
  // Check if code looks like JavaScript
  if (!cleaned.includes('const') && !cleaned.includes('require') && !cleaned.includes('import')) {
    console.error('Generated code preview:', cleaned.substring(0, 500));
    throw new Error('Invalid code generated - does not appear to be JavaScript');
  }

  // Validate required docx usage (more lenient)
  const hasDocxUsage = cleaned.includes('docx') ||
                        cleaned.includes('Document') ||
                        cleaned.includes('Paragraph');

  if (!hasDocxUsage) {
    console.error('Generated code preview:', cleaned.substring(0, 500));
    throw new Error('Invalid code generated - missing docx library usage');
  }

  // Validate it attempts to save a file
  if (!cleaned.includes('Packer') && !cleaned.includes('writeFile')) {
    console.error('Generated code preview:', cleaned.substring(0, 500));
    throw new Error('Invalid code generated - no file output detected');
  }
