...
- Output ONLY valid, executable JavaScript code
- Ensure all arrays, objects, and functions are properly closed
- Validate all brackets are matched: [], {}, ()
- Do NOT truncate the code - complete ALL page processing
- Test that the code has valid JavaScript syntax
- No markdown, no explanations, no comments at the end
- The code must be complete and ready to execute
...
function validateGeneratedCode(code) {
  ...
  // Check for basic syntax errors by attempting to parse
  // This catches common issues like mismatched brackets
  try {
    // Use Function constructor to validate syntax without executing
    // Wrap in async function since generated code may use await
    new Function('return (async function() { ' + cleaned + ' })');
  } catch (syntaxError) {
    ...
    throw new Error(`Syntax error in generated code${lineInfo}: ${errorMessage}`);
  }
