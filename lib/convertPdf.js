...
async function convertPdf(pdfBuffer, jobId, settings, jobManager) {
  ...
  try {
    ...
    const response = await anthropic.messages.create({
      model,
      max_tokens: 32000, // Increased to prevent code truncation
      temperature: 0.3,
      messages: [{
        role: 'user',
        content: [
    ...
    // Save raw generated code for debugging (before validation)
    const scriptPath = path.join(workDir, 'convert.js');
    const rawCodePath = path.join(workDir, 'convert_raw.js');
    fs.writeFileSync(rawCodePath, code);
    log(`Raw code saved to ${rawCodePath}`);
    ...
    try {
      ...
      log(`Validation failed. Code saved to ${scriptPath} for debugging.`);

      throw new Error(`Code validation failed: ${validationError.message}\nGenerated code saved to: ${scriptPath}`);
    }
    ...
    fs.writeFileSync(scriptPath, code);
    log(`Executable code saved to ${scriptPath}`);
    ...
    try {
      ...
      log(`Execution failed. Code available at: ${scriptPath}`);
