...
const MODEL_PRICING = {
  ...
  'claude-opus-4-5-20251101': { input: 5.00, output: 25.00, cache: 0.50 }
};
...
async function convertPdf(pdfBuffer, jobId, settings, jobManager) {
  ...
  const log = (message) => {
    ...
    // Always add to job logs for UI
    jobManager.addLog(jobId, message);
  };
  ...
  try {
    ...
    log(`Skills configured: docx (type: anthropic, version: latest)`);
    ...
    stream.on('text', (textDelta) => {
      // log(`Claude: ${textDelta}`); // Too verbose for main text, maybe just chunks?
    });

    stream.on('content_block_start', (block) => {
      if (block.content_block?.type === 'tool_use') {
        const msg = `🛠️  Tool Use: ${block.content_block.name}`;
        console.log(`[${jobId}] ${msg}`);
        jobManager.addLog(jobId, msg);

        if (block.content_block.input) {
          const inputMsg = `   Input: ${JSON.stringify(block.content_block.input)}`;
          console.log(`[${jobId}] ${inputMsg}`);
          jobManager.addLog(jobId, inputMsg);
        }
      } else if (block.content_block?.type === 'thinking') {
        const msg = `🧠  Thinking...`;
        console.log(`[${jobId}] ${msg}`);
        jobManager.addLog(jobId, msg);
      }
    ...
    stream.on('content_block_delta', (delta) => {
      if (delta.type === 'input_json_delta') {
        // process.stdout.write(delta.partial_json); // Optional: stream json
    ...
    if (!generatedFileId) {
      ...
      console.error(`[${jobId}] ⚠️  SKILL NOT USED - Claude generated text response instead of using docx skill`);
      throw new Error('No DOCX file generated by docx skill');
    }

    log(`✓ File generated by docx skill: ${generatedFileId}`);
...
function buildPrompt(settings) {
  const {
    ...
    recognizeHeadersFooters = false,
    overrideFormatting = false,
    customInstructions = ''
  } = settings;

  // Natural language task description with explicit skill mention
  let prompt = 'Create a Word document from this PDF using the docx skill. Preserve all formatting, tables, and content from every page. IMPORTANT: Ensure all text is in black font color. Do not use any background colors for table cells. Set the document page size to Letter (8.5 x 11 inches).';

  if (recognizeHeadersFooters) {
    prompt += ' Identify content that repeats at the top (headers) and bottom (footers) of the pages. Place this content into the actual Header and Footer sections of the Word document, do not repeat it in the main body text. Ensure the main body text flows continuously across pages without being interrupted by headers/footers.';
  }
...
function buildImagePrompt(settings) {
  ...
  // Natural language task description with explicit skill mention
  let prompt = 'Create a Word document from this image using the docx skill. Extract all text and preserve the formatting. IMPORTANT: Ensure all text is in black font color. Do not use any background colors for table cells. Set the document page size to Letter (8.5 x 11 inches).';
