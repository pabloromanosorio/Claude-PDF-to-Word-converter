...
async function convertPdf(pdfBuffer, jobId, settings, jobManager) {
  ...
  try {
    ...
    // Calculate needed tokens based on PDF size (rough estimate)
    const pdfSizeKB = pdfBuffer.length / 1024;
    const estimatedOutputTokens = Math.min(64000, Math.max(16000, pdfSizeKB * 100));

    log(`PDF size: ${pdfSizeKB.toFixed(1)}KB, requesting up to ${estimatedOutputTokens} tokens`);
    ...
    if (outputTokens >= estimatedOutputTokens * 0.95) {
      log(`WARNING: Used ${outputTokens}/${estimatedOutputTokens} tokens (95%+); response may be truncated`);
      // Save warning for validation to handle
      fs.writeFileSync(path.join(workDir, 'tokens_warning.txt'),
        `Used ${outputTokens}/${estimatedOutputTokens} tokens (95%+). Response may be truncated.`);
    }
