...
const { buildConversionPrompt, buildChunkPrompt, cleanCode, validateGeneratedCode, ensureFileWriting } = require('./converter');
const { analyzeDocument, selectStrategy } = require('./pdfAnalyzer');
const { splitPdfIntoChunks, mergeChunkCodes } = require('./pdfChunker');
...
// Load SKILL files at module level for prompt caching (70%+ cost reduction on subsequent chunks)
let SKILL_1 = null;
let SKILL_2 = null;

try {
  SKILL_1 = fs.readFileSync('/mnt/skills/public/docx/SKILL.md', 'utf8');
  SKILL_2 = fs.readFileSync('/mnt/skills/public/docx/docx-js.md', 'utf8');
  console.log('✓ SKILL files loaded for prompt caching');
} catch (error) {
...
/**
 * Convert PDF in chunks (for large/complex documents)
 * @param {Buffer} pdfBuffer - PDF file buffer
 * @param {Object} strategy - Strategy object with pagesPerChunk
 * @param {Object} settings - Conversion settings
 * @param {string} workDir - Working directory
 * @param {function} log - Logging function
 * @param {function} updateProgress - Progress update function
 * @returns {Promise<Object>} Conversion result with code, tokens, and metrics
 */
async function convertChunked(pdfBuffer, strategy, settings, workDir, log, updateProgress) {
  const { pagesPerChunk, chunks: numChunks } = strategy;

  log(`Strategy: Chunked conversion (${numChunks} chunks × ${pagesPerChunk} pages)`);

  // Split PDF
  updateProgress(25, `Splitting PDF into ${numChunks} chunks`);
  const chunks = await splitPdfIntoChunks(pdfBuffer, pagesPerChunk);
  log(`PDF split into ${chunks.length} chunks`);
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const progressRange = 30; // 30-60% for chunk conversion
    const chunkProgress = progressBase + Math.floor((i / chunks.length) * progressRange);

    updateProgress(chunkProgress, `Converting chunk ${i + 1}/${chunks.length} (pages ${chunk.startPage}-${chunk.endPage})`);
    log(`Converting chunk ${i + 1}/${chunks.length} (pages ${chunk.startPage}-${chunk.endPage})`);

    const chunkStart = Date.now();

    const prompt = buildChunkPrompt(
      chunk,
      path.join(workDir, 'outputs', 'converted.docx'),
      isFirst,
      isLast
    );

    // Build API request with optional prompt caching
    const requestConfig = {
      model,
    ...
    // Add prompt caching if SKILL files are available (70%+ cost reduction on chunk 2+)
    if (SKILL_1 && SKILL_2) {
    ...
    chunkResults.push(cleanedCode);

    log(`  ✓ Chunk ${i + 1} completed in ${(chunkTime / 1000).toFixed(1)}s (${response.usage.output_tokens} tokens)`);

    // Save each chunk for debugging
    fs.writeFileSync(path.join(workDir, `chunk_${i + 1}_raw.txt`), rawCode);
    fs.writeFileSync(path.join(workDir, `chunk_${i + 1}_cleaned.js`), cleanedCode);

    // Rate limit pause between chunks (except after last)
  ...
  // Merge chunks
  updateProgress(65, 'Merging chunks');
  log('Merging chunks...');
  const mergedCode = mergeChunkCodes(chunkResults);
  log(`Merged code: ${mergedCode.length} chars`);
...
async function convertPdf(pdfBuffer, jobId, settings, jobManager) {
  ...
  try {
    ...
    // Step 3: Execute conversion based on strategy
    let rawCode;
    let inputTokens, outputTokens;
    const startTime = Date.now();

    if (strategy.type === 'chunked') {
      // Chunked conversion for large/complex documents
      const updateProgress = (progress, step) => {
        jobManager.updateJob(jobId, { progress, currentStep: step });
      };

      const result = await convertChunked(pdfBuffer, strategy, settings, workDir, log, updateProgress);
      rawCode = result.code;
      inputTokens = result.inputTokens;
      outputTokens = result.outputTokens;

      log(`All chunks converted in ${(result.totalTime / 1000).toFixed(1)}s`);
      log(`Total tokens: ${inputTokens} input, ${outputTokens} output`);

    } else {
      // Single request for small/medium documents
      jobManager.updateJob(jobId, {
        progress: 20,
      ...
      log(`Using adaptive token limit: ${maxTokens} tokens (single request)`);
      ...
      // Add prompt caching if SKILL files are available
      if (SKILL_1 && SKILL_2) {
        requestConfig.system = [
          { type: 'text', text: SKILL_1, cache_control: { type: 'ephemeral' } },
          { type: 'text', text: SKILL_2, cache_control: { type: 'ephemeral' } }
      ...
      rawCode = response.content[0].text;
    }
